...
def evaluate_model(
    ...
    for x, y in dataloader:
        ...
        with torch.no_grad(), autocast(device_type="cuda", dtype=torch.bfloat16):
            logits, loss = inference_and_loss_step(model, x, y, loss_fn)

            total_loss += loss.item()
            total_loss_norm += loss.item()

            batch_count += 1

            pred_token_indices = logits.argmax(dim=-1)
...
def inference_and_loss_step(model, x, y, loss_fn):
